...
public class Enemy2_FSM : FSM
{
    ...
    private void Awake()
    {
        ...
        ChangeState(State.Idle, () => { ani.SetTrigger("Idle"); });
        Invoke("StartTrace", 2);
    }
    ...
    public override void StateAttack()
    {
        ...
        else
        {
            if (isCanAttack)
            {
                ...
                isCanAttack = false;
                target.GetComponent<Friend_FSM>().Demage();
            }
            else
            {
                CD -= Time.deltaTime;
    ...
    public void PlayDie()
    {
        ChangeState(State.Dead, () => { ani.SetTrigger("Die"); });
    }

    public override void StateDead()
    {
        //Debug.Log("enemy is dead!");
        target.GetComponent<Friend_FSM>().target =null;
        enabled = false;
    }
}
